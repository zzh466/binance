...
- 主窗口的“当前持仓（现货 + U 本位永续）”统一展示当前账号的持仓快照：现货列出除 USDT 外总余额不为 0 的资产，U 本位列出 `positionAmt != 0` 的合约，并显示方向、数量、锁定资产或保证金、开仓均价、当前价或标记价、USDT 价值、未实现盈亏、杠杆和保证金模式。自动更新复用每 2 秒的账户快照；账户事件或成交发生后也会触发刷新，用户仍可点击按钮立即重查。
