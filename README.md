...
代码模块职责和修改边界见 [架构说明](docs/ARCHITECTURE.md)。
...
- 主窗口的“当前持仓（现货 + U 本位永续）”统一展示当前账号的持仓快照：现货列出除 USDT 外总余额不为 0 的资产，U 本位列出 `positionAmt != 0` 的合约，并显示方向、数量、锁定资产或保证金、开仓均价、当前价或标记价、USDT 价值、未实现盈亏、杠杆和保证金模式。程序独立地每 2 秒复核一次，账户事件或成交发生后也会提前触发刷新。只有现货和 U 本位查询均成功、快照未过期、程序没有已知未成交订单并连续两次返回零持仓时，页面才显示绿色“已确认无持仓”；任一市场失败、未配置或超过 8 秒没有新快照时，页面明确显示“无法确认”，不会把空白或部分结果误报成空仓。部分市场失败时保留上次成功查到的该市场持仓并标记为旧快照。绿色状态只确认当前持仓，休息前仍应单独核对所有普通订单和条件单均已撤销。
- 登录后每 2 秒执行一次轻量 Binance 账户快照，并通过管理端 `PATCH /future/futureAccountTradingInfo` 回写当前账号的 `id`、静态权益、动态权益、可用资金、保证金、持仓盈亏、平仓盈亏、实际盈亏、手续费、开仓量和报单量。开仓量按当前持仓品种数、报单量按 Binance 已确认的未完成订单数统计，避免把不同币种的小数数量直接相加。同步请求复用登录 Cookie、禁止并发重叠，失败时保留本地交易功能并在下一周期重试；成交由账户 WebSocket 增量维护，完整成交/收益历史每 5 分钟对账一次。
...
- “一键平所有”会暂时暂停后台高频账户查询，分别撤销 Spot 与 USDⓈ-M 挂单，分批卖出存在直接 USDT 交易对的现货资产，并用 `reduceOnly` 市价单平掉 U 本位持仓。明确收到 429 拒绝时会按市场等待限流恢复后续跑；请求状态未知时绝不自动重报。无直接 USDT 交易对、锁定余额及低于交易限制的零头会单独列出，不能被误报为已经清仓。
