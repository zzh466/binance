...
- 全局合约旁可查询并设置当前账号的合约杠杆：根据 Binance 风险档位生成连续倍率选项，以服务器当前值校准，并按环境、账号和合约保存本地记录。
- 行情使用 Binance 原生部分深度流，可在页面顶部切换 5/10/20 档，默认 `@depth20@100ms`；档位表展示当前快照，主行情图和缩放行情图都会在本次连接内半透明保留已经移出当前档位的最后观察值，切换合约、环境、档位、缩放级别或断线重连时清空；最新成交价使用独立逐笔成交流。
- 原行情下方提供只读“缩放行情”：每帧按 1/5/10 级聚合当前已经收到的 5/10/20 档数据，再累计显示本次连接中观察到的聚合价位，不额外订阅更深行情；基础刻度为 `0.01`，区间采用左开右闭并归入上边界。
...
- U 本位交易固定使用单向持仓模式。
- 最近 24 小时订单、交易回合、快捷键、杠杆记录和跨实例限流快照保存在本地 JSON；没有长期数据库。
