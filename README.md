...
- 当前账号的资金指标每 30 秒从 Binance 自动刷新，并在账户成交事件后提前刷新：静态权益为现货资产 USDT 估值加 U 本位钱包余额，动态权益为现货资产 USDT 估值加 U 本位保证金余额，可用资金为现货可用资产 USDT 估值加 U 本位可用余额。U 本位近 24 小时实际盈亏按 `REALIZED_PNL` 收入求和。
- 现货实际盈亏使用持久化的平均成本账本，只由真实成交更新，撤单不产生盈亏。首次跟踪时按当时市价把已有持仓设为成本基线；之后由账户 WebSocket 成交即时增量更新，并通过 `myTrades` 定时去重对账。因此它代表“本程序开始跟踪后的近 24 小时已实现盈亏”，首次跟踪之前的历史买入成本不会被臆测。
- REST 连通性和服务器时间同步。
