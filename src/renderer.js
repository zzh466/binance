...
function getPositionSourceStatus(source, label) {
  if (!source?.configured) return `${label}未配置`;
  return source.ok ? `${label}已核验` : `${label}查询失败`;
}

function getPositionScopeLabel(snapshot = {}) {
  const environmentLabel = snapshot.environment === "production"
    ? "正式环境"
    : "测试环境";
  return snapshot.accountName
    ? `${environmentLabel} / ${snapshot.accountName}`
    : environmentLabel;
}

function renderCurrentPositionsSnapshot(snapshot = {}, { replaceRows = true } = {}) {
  snapshot = mergePositionSnapshots(latestPositionSnapshot, snapshot);
  const safety = evaluatePositionSafety(snapshot, {
    staleMs: POSITION_SNAPSHOT_STALE_MS,
    requiredConfirmations: REQUIRED_FLAT_CONFIRMATIONS,
    previousFlatConfirmations: consecutiveFlatPositionConfirmations,
    previousConfirmedAt: lastFlatPositionConfirmationAt,
  });
  consecutiveFlatPositionConfirmations = safety.flatConfirmations;
  lastFlatPositionConfirmationAt = safety.confirmedAt;
  latestPositionSnapshot = snapshot;

  elements.positionSafetyBanner.className =
    `position-safety-banner ${safety.level}`;
  ...
  const ageText = Number.isFinite(safety.ageMs)
    ? `数据年龄 ${(safety.ageMs / 1000).toFixed(1)} 秒`
    : "没有有效数据时间";
  const statusParts = [
...
function renderCurrentPositionsFailure(error) {
  const failure = {
    name: error?.name || "PositionRefreshError",
    message: error?.message || "刷新失败",
  };
  const previousSources = latestPositionSnapshot?.sources || {};
  renderCurrentPositionsSnapshot({
...
async function refreshCurrentPositions({ showResult = true } = {}) {
  ...
  try {
    ...
    if (!result.ok) {
      renderCurrentPositionsFailure(result.error || {
    ...
    renderCurrentPositionsFailure(error);
  } finally {
...
function updateShortcutSummary() {
  const descriptions = shortcutSettings.map((shortcut) => {
    ...
    if (!shortcutApi.isOrderAction(shortcut.action)) {
      return `${key} = ${shortcutApi.getActionLabel(shortcut.action)}`;
    }
...
function openShortcutCreator() {
  ...
  if (!availableKey) {
    window.alert("所有可用按键均已配置，请先删除或修改一个现有快捷键。");
    return;
  }
...
async function closeAllPositionsFromShortcut(shortcut) {
  const shortcutLabel = shortcutApi.getKeyLabel(shortcut.key);
  ...
  const details = result.ok ? result.data : result.error?.data;
  for (const order of details?.orders || []) applyOpenOrderUpdate(order);
  printResult(`${shortcutLabel} 一键平所有结果`, result);
...
document.addEventListener("keydown", async (event) => {
  ...
  try {
    ...
    if (shortcut.action === shortcutApi.ACTION_CLOSE_ALL_POSITIONS) {
      await closeAllPositionsFromShortcut(shortcut);
    }
  } catch (error) {
...
setInterval(() => {
  if (!latestPositionSnapshot) return;
  renderCurrentPositionsSnapshot(latestPositionSnapshot, { replaceRows: false });
}, 1_000);
...
window.binance.onCloseAllProgress?.((progress = {}) => {
  printResult("一键平所有进度", {
    ok: true,
    data: progress,
  });
