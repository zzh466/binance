...
- `src/binance/binanceSpotClient.js`：现货行情、交易、账户事件和网络降级。
- `src/binance/binanceUsdMClient.js`：U 本位行情、交易、账户事件和网络降级。
- `src/binance/binanceUnifiedClient.js`：按合约类型在现货与 U 本位之间统一路由。
...
2. 现货与 U 本位订单必须通过统一客户端路由，页面不直接拼接 Binance 请求。
